...
{
    [InitializeOnLoad]
    public class theblackarmsSDX_ImportPanel_Addons : EditorWindow
    {
        ...
        [MenuItem("theblackarmsSDX/Import panel/addons", false, 501)]
        public static void OpenImportPanel()
        {
            GetWindow<theblackarmsSDX_ImportPanel_Addons>(true);
        }

        public void OnEnable()
        {
            titleContent = new GUIContent("theblackarmsSDX Import panel addons");

            theblackarmsSDX_ImportManager_Addons.checkForConfigUpdate();
            LoadJson();
        ...
        public static void LoadJson()
        {
            ...
            dynamic configJson =
                JObject.Parse(File.ReadAllText(theblackarmsSDX_Settings.projectConfigPath + theblackarmsSDX_ImportManager_Addons.configName));

            Debug.Log("Server Asset Url is: " + configJson["config"]["serverUrl"]);
            theblackarmsSDX_ImportManager_Addons.serverUrl = configJson["config"]["serverUrl"].ToString();
            ...
            foreach (var asset in assets)
            {
                ...
                else
                {
                    if (GUILayout.Button(
                        ...
                        " " + asset.Key))
                    {
                        theblackarmsSDX_ImportManager_Addons.downloadAndImportAssetFromServer(asset.Value);
                    }

                    if (GUILayout.Button("Del", GUILayout.Width(40)))
                    {
                        theblackarmsSDX_ImportManager_Addons.deleteAsset(asset.Value);
                    }
                }
                GUILayout.EndHorizontal();
            }

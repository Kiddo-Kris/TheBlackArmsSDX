...
{
    [InitializeOnLoad]
    public class theblackarmsSDX_ImportPanel_Shaders : EditorWindow
    {
        ...
        [MenuItem("theblackarmsSDX/Import panel/Shaders", false, 501)]
        public static void OpenImportPanel()
        {
            GetWindow<theblackarmsSDX_ImportPanel_Shaders>(true);
        }

        public void OnEnable()
        {
            titleContent = new GUIContent("theblackarmsSDX Import panel shaders");

            theblackarmsSDX_ImportManager_Shaders.checkForConfigUpdate();
            LoadJson();
        ...
        public static void LoadJson()
        {
            ...
            dynamic configJson =
                JObject.Parse(File.ReadAllText(theblackarmsSDX_Settings.projectConfigPath + theblackarmsSDX_ImportManager_Shaders.configName));

            Debug.Log("Server Asset Url is: " + configJson["config"]["serverUrl"]);
            theblackarmsSDX_ImportManager_Shaders.serverUrl = configJson["config"]["serverUrl"].ToString();
            ...
            foreach (var asset in assets)
            {
                ...
                else
                {
                    if (GUILayout.Button(
                        ...
                        " " + asset.Key))
                    {
                        theblackarmsSDX_ImportManager_Shaders.downloadAndImportAssetFromServer(asset.Value);
                    }

                    if (GUILayout.Button("Del", GUILayout.Width(40)))
                    {
                        theblackarmsSDX_ImportManager_Shaders.deleteAsset(asset.Value);
                    }
                }
                GUILayout.EndHorizontal();
            }

...
{
    [InitializeOnLoad]
    public class theblackarmsSDX_ImportPanel_Avatars : EditorWindow
    {
        ...
        [MenuItem("theblackarmsSDX/Import panel/Avatars", false, 501)]
        public static void OpenImportPanel()
        {
            GetWindow<theblackarmsSDX_ImportPanel_Avatars>(true);
        }

        public void OnEnable()
        {
            titleContent = new GUIContent("theblackarmsSDX Import panel avatars");

            theblackarmsSDX_ImportManager_Avatars.checkForConfigUpdate();
            LoadJson();
        ...
        public static void LoadJson()
        {
            ...
            dynamic configJson =
                JObject.Parse(File.ReadAllText(theblackarmsSDX_Settings.projectConfigPath + theblackarmsSDX_ImportManager_Avatars.configName));
            ...
            foreach (var asset in assets)
            {
                ...
                else
                {
                    if (GUILayout.Button(
                        ...
                        " " + asset.Key))
                    {
                        theblackarmsSDX_ImportManager_Avatars.downloadAndImportAssetFromServer(asset.Value);
                    }

                    if (GUILayout.Button("Del", GUILayout.Width(40)))
                    {
                        theblackarmsSDX_ImportManager_Avatars.deleteAsset(asset.Value);
                    }
                }
                GUILayout.EndHorizontal();
            }

...
{
    [InitializeOnLoad]
    public class theblackarmsSDX_ImportPanel_Prefabs : EditorWindow
    {
        ...
        [MenuItem("theblackarmsSDX/Import panel/Prefabs", false, 501)]
        public static void OpenImportPanel()
        {
            GetWindow<theblackarmsSDX_ImportPanel_Prefabs>(true);
        }

        public void OnEnable()
        {
            titleContent = new GUIContent("theblackarmsSDX Import panel prefabs");

            theblackarmsSDX_ImportManager_Prefabs.checkForConfigUpdate();
            LoadJson();
        ...
        public static void LoadJson()
        {
            ...
            dynamic configJson =
                JObject.Parse(File.ReadAllText(theblackarmsSDX_Settings.projectConfigPath + theblackarmsSDX_ImportManager_Prefabs.configName));

            Debug.Log("Server Asset Url is: " + configJson["config"]["serverUrl"]);
            theblackarmsSDX_ImportManager_Prefabs.serverUrl = configJson["config"]["serverUrl"].ToString();
            ...
            foreach (var asset in assets)
            {
                ...
                else
                {
                    if (GUILayout.Button(
                        ...
                        " " + asset.Key))
                    {
                        theblackarmsSDX_ImportManager_Prefabs.downloadAndImportAssetFromServer(asset.Value);
                    }

                    if (GUILayout.Button("Del", GUILayout.Width(40)))
                    {
                        theblackarmsSDX_ImportManager_Prefabs.deleteAsset(asset.Value);
                    }
                }
                GUILayout.EndHorizontal();
            }
